...
def true_gain_list(predicted_xds_list, scalar_mode):
    ...
    for xds in predicted_xds_list:
        ...
        phase = 2*np.pi*delays*origin_chan_freq + 2*np.pi*tec*(1/origin_chan_freq) + offsets
        gains = amp*da.exp(1j*phase)

        # TODO: This is only correct for 4 correlation data.
        params = da.zeros(
            (n_time, 1, n_ant, n_dir, 6),
            chunks=param_chunking[:-1] + (6,)
        )
        params[..., 0] = offsets[..., 0]
        params[..., 3] = offsets[..., -1]
        params[..., 1] = tec[..., 0]
...
def test_params(cmp_gain_xds_lod, true_gain_list):
    ...
    for solved_gain_dict, true_gain_xds in zip(cmp_gain_xds_lod, true_gain_list):
        ...
        cf = solved_gain_xds.gain_freq.values
        # sel = np.where((solved_params - true_params) > 100)
        # sp = solved_params[sel[:-1]][0]
        # tp = true_params[sel[:-1]][0]

        # wrap_number = sp[4] / cf.min() - sp[4] / cf.max()
        # print("WRAP_NUMBER ", wrap_number)
        # wrap_number = sp[5] * cf.max() - sp[5] * cf.min()
        # print("WRAP_NUMBER ", wrap_number)
        # wrap_number = tp[4] / cf.min() - tp[4] / cf.max()
        # print("WRAP_NUMBER ", wrap_number)
        # wrap_number = tp[5] * cf.max() - tp[5] * cf.min()
        # print("WRAP_NUMBER ", wrap_number)

        delay_wrap_number = true_params[...,2::3] * cf.max() - true_params[...,2::3] * cf.min()
