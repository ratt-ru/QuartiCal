...
def make_model(ms_xds, opts):
    ...
    # TODO: Add behaviour for leading operation.
    ...
    for xds, prediction in zip(ms_xds, predict_schemes):
        ...
        for recipe in opts._internal_recipe.values():
            ...
            # If we have operations, we loop over them to contruct the model.
            ...
            for op_idx, op in enumerate(operations):
                ...
                if op_idx == 0:
                    if ingredients[op_idx] in prediction.keys():
                        result = prediction.get(ingredients[op_idx])
                    else:
                ...
                if len(in_b) > 1 and len(result) > 1:
                    ...
                    result, in_b = in_b, result  # Result must always be >.
                    op(result[0], in_b[0], out=result[0])
