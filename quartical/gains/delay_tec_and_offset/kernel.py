...
def nb_delay_tec_and_offset_solver_impl(
    ...
    def impl(
        ...
        active_params[..., 2::3] *= 1e9 # mean_freq
        mean_ifreq = (1/chan_freq).mean()
        active_params[..., 1::3] /= 1e9 # mean_ifreq
        ...
        for i in prange(n_int):
            ...
            for row_ind in range(rs, re):
                ...
                for f in range(fs, fe):
                    ...
                    delay_coeff = \
                        2 * np.pi * (chan_freq[f]/1e9)
                    tec_coeff = \
                        2 * np.pi * (1e9/chan_freq[f])
...
def delay_tec_and_offset_params_to_gains(
    ...
    for t in range(n_time):
        for f in range(n_freq):
            ...
            delay_coeff = \
                2 * np.pi * (chan_freq[f] / delay_denom)
            # Coeff associated with the the tec.
            tec_coeff = \
                2 * np.pi * (tec_numerator / chan_freq[f])
            for a in range(n_ant):
                for d in range(n_dir):
