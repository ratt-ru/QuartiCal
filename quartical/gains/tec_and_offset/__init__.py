...
class TecAndOffset(ParameterizedGain):
    ...
    def init_term(self, term_spec, ref_ant, ms_kwargs, term_kwargs):
        ...
        # considering TEC, even though estimating for both clock and TEC.
        gain_tuple = (gains,)
        ...
        for ut in utint:
            ...
            for uf in ufint:
                ...
                path0 = path00+path01
                np.save(path0+"delayest2.npy", delay_est)
                np.save(path0+"delay_fftarr2.npy", fft_datak)
                np.save(path0+"delay_fft_freq2.npy", fft_freqk)

                np.save(path0+"tecest.npy2", tec_est)
                np.save(path0+"tec_fftarr2.npy", fft_datat)
                np.save(path0+"tec_fft_freq2.npy", fft_freqt)
