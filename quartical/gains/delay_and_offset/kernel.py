...
def nb_compute_jhj_jhr(
    ...
    # frequency-dependent exponent, so it also carries the same per-channel
    # coefficient computed by the stage hook; that coefficient is concatenated
    # onto the residual's auxiliary values to form the aux tuple consumed by the
    # elem. The offset parameter adds a second (frequency-independent) parameter
...
def stage_factory(corr_mode):
    ...
    returns it as a single-element flat tuple (coeff,); the shared loop
    concatenates it onto the residual's auxiliary values to form the aux tuple
    passed to the elem.
    """
...
def compute_jhwj_jhwr_elem_factory(corr_mode):
    ...
    frequency independent (its Jacobian carries no coeff), while the delay
    parameter's Jacobian carries the coeff. This yields the interleaved
    (offset, delay) accumulator layout, with jhj entries scaled by 1, coeff or
    coeff**2 depending on which parameter pair the entry couples. The normf
